...
void storeblocks() {
  ...
  topRoller.spin(vex::reverse, 2, vex::volt);
}
void scorelong() {
  conveyor.spin(vex::fwd, 12, vex::volt);
...
void outtake() {
  conveyor.spin(vex::reverse, 12, vex::volt);
  topRoller.spin(vex::reverse, 12, vex::volt);
}
void stop() {
  conveyor.stop();
  topRoller.stop();
...
void forward4life(){
  ...
  chassis.turn_to_angle(0, 8, 1, 200, 900);
  chassis.drive_to_point(24, 24, 6, 9, 6);
  loader.set(true);
  vex::task lowerwing([]{
    wing.set(true);
